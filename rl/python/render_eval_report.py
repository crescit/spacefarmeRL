...
def episode_table(rows: list[dict[str, Any]]) -> str:
    body = []
    for row in rows:
    ...
    return "".join(body) or '<tr><td colspan="8">No completed episodes.</td></tr>'
...
<details><summary>Per-seed trajectories</summary><div class="scroll"><table><thead><tr><th>Seed</th><th>Reward</th><th>Credits</th><th>Steps</th><th>Latency</th><th>Capped</th><th>Replay</th><th>Trajectory</th></tr></thead><tbody>{episode_table(rows)}</tbody></table></div></details><footer>Source: <code>{esc(report["_path"])}</code></footer></article>"""
