// preview_characters.mjs — render the painterly player + NPC cast up-close
import * as SS from '../client/systems/SpriteSystem.js';
import { px } from '../client/systems/SpriteSystem.js';
import zlib from 'node:zlib';
import fs from 'node:fs';

const SC =  6;             // upscale for eyeballing
const ids = ['player', 'nova', 'luna', 'zephyr', 'vega', 'quasar', 'rhea', 'astra', 'orion', 'comet', 'cora'];
const COLS = 6;
const CW = 360, CH = 420;    // cell (padded, sprite 64x80 * SC)
...
const blit = (cv, ox, oy) => {
  // center 64x80 sprite in its 72x88 cell, scaled SC
  const cx = ox + (CW - cv.width * SC) / 2, cy = oy + (CH - cv.height * SC) / 2;
  for (let y = 0; y < cv.height; y++) for (let x = 0; x < cv.width; x++) {
    const p = px(cv, x, y); if (p[3] === 0) continue;
    for (let sy = 0; sy < SC; sy++) for (let sx = 0; sx < SC; sx++) put(cx + x * SC + sx, cy + y * SC + sy, p);
  }
  // dark backdrop per cell
};
// transparent-friendly: fill light backdrop first
for (let y = 0; y < H; y++) for (let x = 0; x < W; x++) { const i = (y * W + x) * 4; fb[i] = 30; fb[i + 1] = 34; fb[i + 2] = 50; fb[i + 3] = 255; }
ids.forEach((id, idx) => {
  const tex = id === 'player' ? SS.TEXTURES['player.front_0'] : SS.TEXTURES[`npc.${id}_0`];
  blit(tex, (idx % COLS) * CW, Math.floor(idx / COLS) * CH);
});
// PNG encode
...
fs.writeFileSync('/tmp/characters.png', Buffer.concat([sig, chunk('IHDR', ihdr), chunk('IDAT', zlib.deflateSync(raw)), chunk('IEND', Buffer.alloc(0))]));
console.log(`wrote /tmp/characters.png (${W}x${H})`);
